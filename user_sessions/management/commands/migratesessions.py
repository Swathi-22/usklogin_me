...
def get_model_class(full_model_name):
    try:
        ...
        if 'INSTALLED_APPS' in e.message:
            raise RuntimeError("To run this command, temporarily append '{model}' to settings.INSTALLED_APPS".format(model=old_model_package.rsplit('.models')[0]))
        raise
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--oldmodel', dest='oldmodel', default='django.contrib.sessions.models.Session', help='Existing session model to migrate to the new UserSessions database table'
        )
        ...
        for old_session in old_sessions:
            if not UserSession.objects.filter(session_key=old_session.session_key).exists():
                ...
                UserSession.objects.create(
                    session_key=old_session.session_key, session_data=old_session.session_data, expire_date=old_session.expire_date, user=user, ip='127.0.0.1'
                )
                conversion_count += 1
